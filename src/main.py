...
def get_season(bdl_id, season_year):
    response = requests.get("https://www.balldontlie.io/api/v1/season_averages?player_ids[]=" + str(bdl_id) + "&season=" + str(season_year))
    if response.status_code == 200:
        if json.loads(response.content)["data"]:
            season_to_add = json.loads(response.content)["data"][0]
            season_to_add["season"] = f"{season_year}-{season_year+1}"
            return season_to_add
    return None


def get_players_career_stats(player_name):
    # get player id from balldontlie api
    response = requests.get("https://www.balldontlie.io/api/v1/players?search=" + player_name)
    if response.status_code == 200:
        bdl_id = json.loads(response.content)["data"][0]["id"]
    else:
        return None
    ...
    with FuturesSession() as session:
        futures = [get_season(bdl_id, season_year) for season_year in range(1996, year)]
...
def player(player_id):
    ...
    career_stats = get_players_career_stats(player_name)
    if career_stats:
